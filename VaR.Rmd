...
My recent article focused on using R to perform some basic exploratory data analysis.   
...
For this article we will focus on Value at Risk[^1], a common market risk measure developed by JP Morgan and most recently criticized by Nassim Taleb .

**Simple Historical Simulation Methodology**

For the first part of this article I will walk through the methodology of calculating VaR for a single-stock using the historical simulation method (as opposed to the Monte Carlo or parametric method). 
...
V will be the Value at Risk; N will be the horizon, and X the confidence level.  
...
shinyUI(
  ...
  fluidPage(
    ...
    # Generate a row with a sidebar, calling the sidebar "Instrument" and populating the choices with the content of the vector SYMs
...
shinyServer(function(input, output){
  ...
  output$VaRPlot<-renderPlot({
    ...
    ##this time use the quantmod package that creates the histogram and adds 95% VaR using the add.risk method
...
<img src="figures/shiny_app.tiff" />
